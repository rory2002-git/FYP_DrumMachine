...
{
    // Hardcoding the drumset file path
    QString drumsetPath = "/home/rory/Documents/BBWorkspace/user_lib/drum_sets/Brushes Drumset v2.0.DRM";
   
    qDebug() << "Loading drumset " << drumsetPath;

    QFile file(drumsetPath);
    if (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "Failed to open drumset file at: " << drumsetPath;
        return;
    }
...
{
    // Hardcoding the song file path
    QString songPath = "/home/rory/Documents/BBWorkspace/user_lib/projects/BeatBuddy Default Content 2.0 - Project/SONGS/203A18C4/716D6763.BBS";
   
    qDebug() << "Loading song " << songPath;

    QFile file(songPath);
    if (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "Player::loadSong - ERROR 1 - unable to find " << songPath;
        return false;
    }
